...
public class CreateTenantSteps : TenantStepsBase
{
    ...
    public async Task GivenICreateAWellKnownChildOfTheRootTenantCalledWithTheDetailsAvailableAsAsync(string wellKnownGuidLabel, string tenantName, string newTenantLabel)
    {
        ...
        string parentTenantLabel, string newTenantName, string newTenantLabel)
    {
        ...
        ITenant newTenant = await this.TenantStore.CreateChildTenantAsync(parent.Id, newTenantName);
        this.Tenants.Add(newTenantLabel, newTenant);
        this.AddTenantToDelete(newTenant.Id);
    }

    [When(@"I create a well known \(from the Guid labelled '([^']*)'\) child of the tenant labelled '([^']*)' named '([^']*)' labelled '([^']*)'")]
    public async Task GivenICreateAWellKnownChildOfAChildOfATenantAsync(string wellKnownGuidLabel, string parentTenantLabel, string newTenantName, string newTenantLabel)
    {
        Guid wellKnownGuid = this.WellKnownGuids[wellKnownGuidLabel];
        ITenant parent = this.Tenants[parentTenantLabel];
    ...
    public async Task GivenITryToCreateAWellKnownChildOfAChildOfATenantAsync(string wellKnownGuidLabel, string parentTenantLabel, string newTenantName)
    {
        Guid wellKnownGuid = this.WellKnownGuids[wellKnownGuidLabel];
        ITenant parent = this.Tenants[parentTenantLabel];
    ...
    public void TenantShouldHaveStorageConfigEquivalentToRoot(string tenantLabel)
    {
        ...
        bool propagateRootAsV2 = this.SetupMode is
            ...
            MultiMode.SetupModes.DirectToStoragePropagateRootConfigAsV2;
