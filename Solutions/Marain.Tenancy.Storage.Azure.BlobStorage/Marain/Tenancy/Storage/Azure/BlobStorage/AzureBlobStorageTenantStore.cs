    ...
    IPropertyBagFactory propertyBagFactory) : ITenantStore
{
    ...
    private async Task<(ITenant ParentTenant, BlobContainerClient Container)> GetContainerAndTenantForChildTenantsOfAsync(string tenantId)
    {
        ...
        if (tenantId == RootTenant.RootTenantId)
        {
            await LazyInitializer.EnsureInitialized(ref this.rootContainerExistsCheck, async () => await blobContainer.CreateIfNotExistsAsync().ConfigureAwait(false));
    ...
    {
        return await this.containerSource.GetBlobContainerClientFromTenantAsync(tenant, TenancyV2ConfigKey, TenancyV3ConfigKey, TenancyContainerName).ConfigureAwait(false);
    }
        ...
        string tenantId, BlobContainerClient container, string? etag)
    {
        ...
        {
            tenant = await System.Text.Json.JsonSerializer.DeserializeAsync<Tenant>(
                sr.BaseStream,
        ...
        if (tenant is null)
        {
            throw new System.Text.Json.JsonException($"Deserialization of tenant '{tenantId}' from blob '{blob.Uri}' resulted in a null object.");
        }
